...
# title. Occurrence count = 1 (the filing) + N (comments whose body starts
# with "Occurrence:") — must match the retro skill's Candidate ledger
# definition; ordinary discussion comments never increment the count. Age is
# floored days since createdAt, computed from epoch seconds portably inside
# gh's jq.
list_candidates() {
